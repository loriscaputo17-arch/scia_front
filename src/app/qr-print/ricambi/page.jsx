...
export default function QRPrintPage() {
  ...
  const [selected, setSelected] = useState(new Set()); // IDs selezionati per stampa
  const [selectMode, setSelectMode] = useState(false); // modalità selezione manuale

  useEffect(() => {
    const fetchData = async () => {
      if (!user?.id) return;
      try {
        const result = await fetchSpares(shipId);
        setElements(result || []);
        setLoading(false);
      } catch (error) {
  ...
  const filtered = useMemo(() => {
    ...
    return elements.filter(
      (el) =>
        el.Part_name?.toLowerCase().includes(q) ||
        el.Serial_number?.toLowerCase().includes(q) ||
        el.element_model?.ESWBS_code?.toLowerCase().includes(q)
    );
  }, [elements, search]);
  ...
  return (
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .toolbar {
          ...
          z-index: 100;
          background: #022a52;
          border-bottom: 1px solid rgba(120,159,214,0.2);
          padding: 14px 24px;
          display: flex;
          align-items: center;
          gap: 12px;
        ...
        .search-input {
          ...
          border: 1px solid rgba(120,159,214,0.25);
          color: white;
        ...
        .btn-ghost {
          ...
          border: 1px solid rgba(120,159,214,0.35);
          color: #789fd6;
        }
        ...
        .stats-bar {
          ...
          border-bottom: 1px solid rgba(120,159,214,0.1);
          display: flex;
          align-items: center;
          gap: 20px;
          font-family: 'Share Tech Mono', monospace;
          font-size: 10px;
          color: rgba(120,159,214,0.7);
          letter-spacing: 0.1em;
        }
        .stats-bar span { display: flex; align-items: center; gap-6: 6px; }
        ...
        .qr-card.is-selected .select-check {
          background: #789fd6;
        }
        ...
        .qr-img-wrapper img { width: 140px; height: 140px; display: block; }
        .qr-card-name {
          font-size: 11px;
          font-weight: 700;
          text-align: center;
      ...
      <div className="toolbar">
        <div>
          <div className="toolbar-title">QR Code Ricambi</div>
        ...
        <div className="toolbar-right">
          <button className="btn btn-primary" onClick={handlePrint}>
            Stampa
            {selectMode && selected.size > 0 && ` (${selected.size})`}
          </button>
        </div>
      </div>
      ...
      <div className="grid-wrapper">
        {filtered.length === 0 ? (
          ...
          filtered.map((el) => {
            ...
            const url = `${BASE_APP_URL}/dashboard/impianti/${el.Serial_number}`;
            const qrSrc = `https://api.qrserver.com/v1/create-qr-code/?size=140x140&data=${encodeURIComponent(url)}&margin=6&color=001c38`;

            return (
              <div
                key={el.ID}
                className={`qr-card ${selectMode ? "selectable" : ""} ${isSelected ? "is-selected" : ""} ${isDeselected ? "is-deselected" : ""}`}
              ...
              >
                ...
                <div className="qr-card-header">
                  <span className="qr-card-header-label">SCIA · RICAMBIO</span>
                  <span className="qr-card-header-eswbs">
                    {el.element_model?.ESWBS_code || "—"}
                  </span>
                </div>

                <div className="qr-card-body">
                  <div className="qr-img-wrapper">
                    <img src={qrSrc} alt={`QR ${el.Part_name}`} />
                  </div>
                  <div className="qr-card-name">{el.Part_name || "—"}</div>
                  <div className="qr-card-serial">S/N · {el.Serial_number}</div>
