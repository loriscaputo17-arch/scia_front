...
const SpareRow = ({ data, onRemove }) => {
  const spare = data?.spare ?? data?.Spare;
  ...
  const handleRowClick = () => {
    router.push(`/dashboard/spare/${spare.ID}`);
  };
  ...
  const handleRemoveProduct = async () => {
    ...
    try {
      const response = await removeProductFromCart(spare.ID, user.id);
      ...
      >
        <div onClick={handleRowClick} className="border border-[#001c38] p-3 flex flex-col justify-center min-h-[60px]" style={{ height: "-webkit-fill-available" }}> 
          <p className="text-white text-[18px] font-semibold truncate">
            {spare?.Part_name || spare?.elementModel?.LCN_name || "Nome non disponibile"}
          </p>
        </div>
        ...
        >
          <QuantityControl quantity={data.quantity} spare_id={spare.ID} />        
          </div>
        <div
